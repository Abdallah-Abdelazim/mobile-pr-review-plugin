...
- **Redundant/duplicated state**: two properties or state variables this diff adds that must always agree (e.g. a list plus a separately-tracked `isEmpty`/`count`, or a raw value plus a cached "formatted" copy of it) instead of one held value and the other derived from it on read — an invitation for the two to drift out of sync.
- **Efficiency smells, scoped to what this diff changed**: repeated work the diff could cache, hoist, or share instead (recomputing the same value, re-parsing, re-fetching data already available in scope — see the platform reference's Performance section for per-platform patterns like hoisting a `DateFormatter`/regex out of a loop); independent operations run sequentially with no data dependency between them where the diff could run them concurrently at negligible risk — Kotlin: back-to-back `await`/blocking calls inside a function that could instead be parallel `launch`/`async` calls inside a `coroutineScope`/`supervisorScope`; Swift: back-to-back `await`s with no dependency between them that `async let` or `withTaskGroup` could run in parallel. Flag only where the diff itself introduces or extends the sequential/repeated shape — not a pre-existing pattern the diff merely touches in passing.

**Part 2 — Software-engineering excellence:**
- **Design principles, applied pragmatically** (flag violations that will cost, not theory): single responsibility, open-for-extension (a `when`/`switch` on a type code extended for the Nth time — wants polymorphism/a sealed hierarchy), dependency direction (domain logic doesn't import UI/framework types), honest interfaces, composition over inheritance for new hierarchies, YAGNI (speculative abstraction with one implementation and no real seam need), leaky abstractions (an internal implementation detail — a Room/Core Data entity, a raw network DTO, a platform-specific type — escaping through a public/domain-facing API instead of being mapped at the boundary, forcing callers to know about a layer they shouldn't).
